    ...
    internal class AccountStatementSqlClausesBuilder {
        ...
        static private string GetFields() {
            return "'-1' AS NUMERO_VERIFICACION, ID_MAYOR, ID_MONEDA, ID_CUENTA_ESTANDAR, ID_SECTOR, ID_TRANSACCION, ID_ELABORADA_POR, " +
                   ...
                   "NUMERO_CUENTA_AUXILIAR, NOMBRE_CUENTA_AUXILIAR, NUMERO_TRANSACCION, NATURALEZA, FECHA_AFECTACION, FECHA_REGISTRO, " +
        ...
        private string GetAccountsChartFilter() {
            ...
            if (_accountStatementQry.BalancesQuery.AccountsChartUID != string.Empty) {

                return $"ID_TIPO_CUENTAS_STD = {AccountsChart.Parse(_accountStatementQry.BalancesQuery.AccountsChartUID).Id}";
            } else {
        ...
        private string GetDateFilter() {
            ...
            return $"FECHA_AFECTACION >= " +
                   $"{DataCommonMethods.FormatSqlDbDate(_accountStatementQry.BalancesQuery.InitialPeriod.FromDate)} " +
                   $"AND FECHA_AFECTACION <= " +
                   $"{DataCommonMethods.FormatSqlDbDate(_accountStatementQry.BalancesQuery.InitialPeriod.ToDate)}";
        }
